...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Tá processando fdp calma ai🤬🇧🇼⌛',
				success: '✔️ Sucesso ✔️',
                                levelon: '❬ ✔ ❭ *permitir o nivelamento*', 
				leveloff: ' ❬ X ❭  *desabilitar nivelamento*',
				levelnoton: '❬ X ❭ *a nota de nivelamento não está ativa*',
				...
				only: {
					...
					ownerG: ' Este comando s� pode ser usado pelo propriet�rio grupo',
					ownerB: 'Este comando s� pode ser usado pelo proprietario do bot!',
					admin: 'KKK MEMBRO COMUM TENTANDO USAR COMANDO DE ADM',
					Badmin: 'O BOT PRECISA SER ADM DO GRUPO PARA EXECUTAR ESSA A��O',
                                        daftarB: ` Registre-se\nOla mano !\nVoce n�o esta registrado, \n\nComando : ${prefix}daftar nama|umur\nDigite : .daftar nome| ${prefix}`,
				}
			}
    			const apakah = ['Sim n�o']
        		const bisakah = ['Sim, voc� n�o pode']
		        const kapankah =  ['Dias de atraso ',' Mais domingo ',' Mais meses ',' Mais anos']
			const botNumber = client.user.jid
			const ownerNumber = ["554398434473@s.whatsapp.net"] // replace this with your number
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'criador':
                  client.sendMessage(from, {displayname: "Tony", vcaurd: vcaurd}, MessageType.contact, { quoted: mek})
